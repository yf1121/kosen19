...
if (flag == true && imgfold.substr (-4) != "null" && tagfold.substr (-4) != "null")  //CSVファイルが開けかつ指定フォルダの末尾4文字がnullでなければ、処理を始める
{
    ...
    if (Style5==null){
        ...
        Style5.pointSize = "8.5Q";  //プロパティ（フォントサイズ）設定
        Style5.leading = "11.5H";  //プロパティ（行送り）設定
            ...
            for (j=0; j<3; j++)
            {
                if (fileObj.eof){
                    ...
                    if (data[3].slice(0, -1) != pageno){  //改ページか確認
                        ...
                        if (data[4].slice(0, -3).toString() != tentno){
                            tentno = data[4].substr(0, 1)  //テントアルファベットの更新
                            if (j == 0){
                                i = i + 0.1
                            }else{
                                i = i + 1.1
                        ...
                        if (data[4].slice(0, 2).toString() != tentno){
                            tentno = data[4].slice(0, 2)  //テントアルファベットの更新
                            if (j == 0){
                                i = i + 0.1
                            }else{
                                i = i + 1.1
                    ...
                    txtObj.textFramePreferences.verticalJustification = VerticalJustification.centerAlign  //フレーム内天地中央揃え
                    /***企画分類アイコン*****************************************************/
                    imgpath = imgfold.replace(/\\/g, '/') + "/" + data[34].slice(0, -1)  //パスの指定//企画分類アイコンデータの取得
                    selFile = new File(imgpath);  //入れる画像ファイル指定
                    imgObj = pageObj.textFrames.add();
                    imgObj.name = data[4] + data[1].slice(0, -1) + "-Icon";  //レイヤー名の設定
                    x51 = x + 2.5;  //左からの座標, 横の間隔
                    y51 = y +20.1;  //上からの座標
                    x52 = x51 + 8;  //横8㎜
                    y52 = y51 + 7.2;  //縦7.2㎜
                    x51 = x51 + "mm";
                    ...
                    if (data[15].slice(0, -1) == "〇"){  //もし学術枠なら、末尾にリンクをはる
                        var linkpage = data[32].slice(0, -1)  //リンクページ変数の設定
                        linkpage = parseInt(linkpage) + 35  //学術扉ページ番号の加算
                        data[19] = data[19].slice(0, -1) + "(→p." + linkpage + ")"  //リンクをはる
                    ...
                    for (k=0; k<3; k++){
                        for (l=0; l<2; l++){
                            ...
                            selFile = new File(imgpath);  //入れる画像ファイル指定
                            imgObj = pageObj.textFrames.add();
                            imgObj.name = data[4] + data[1].slice(0, -1) + "-Tag" + k.toString();  //レイヤー名の設定
                            x51 = x + 4.5 * l + 11.5;  //左からの座標, 横の間隔
                            y51 = y + 4.5 * k + 15.12;  //上からの座標
                            x52 = x51 + 3.86;  //横3.86㎜
                            y52 = y51 + 3.86;  //縦3.86㎜
                            x51 = x51 + "mm";
                            y51 = y51 + "mm";
                    ...
                    /***学実委アイコン*********************************************************/
                    for (k=0; k<3; k++){
                        if (data[20+k].slice(0, -1) == ""){break;break;}
                        imgpath = tagfold.replace(/\\/g, '/') + "/" + data[20+k].slice(0, -1)  //パスの指定//学実委アイコンデータの取得
                        selFile = new File(imgpath);  //入れる画像ファイル指定
                        imgObj = pageObj.textFrames.add();
                        imgObj.name = data[4] + data[1].slice(0, -1) + "-jitsui" + k.toString();  //レイヤー名の設定
                        if (k == 2){k += 2}  //アイコンが3つ目の場合
                        x61 = x + 4.5 * k + 2.5;  //左からの座標, 横の間隔
                        y61 = y + 15.12;  //上からの座標
                        x62 = x61 + 3.86;  //横3.86㎜
                        y62 = y61 + 3.86;  //縦3.86㎜
                        x61 = x61 + "mm";
                        ...
                        x62 = x62 + "mm";
                        y62 = y62 + "mm";
                        imgObj.visibleBounds = [y61,x61,y62,x62];  //新規フレーム[x,y,z,w] => 上からx、左からyに、縦z-x、横w-yの長方形を作る
                        imgObj.contentType = ContentType.graphicType;  //フレームの種類をグラフィックに明示的に設定
                        imgObj.place(selFile);  //入れる画像
                        imgObj.fit(FitOptions.proportionally);  //内容にフレームサイズを合わせる
                    }
                    /***日付アイコン***********************************************************/
                    for (k=0; k<3; k++){
                        m = k + 2  //日の指定
                        imgpath = tagfold.replace(/\\/g, '/') + "/date" + m + ".ai"  //パスの指定//日付アイコンデータの取得
                        selFile = new File(imgpath);  //入れる画像ファイル指定
                        imgObj = pageObj.textFrames.add();
                        imgObj.name = data[4] + data[1].slice(0, -1) + "-date" + m.toString();  //レイヤー名の設定                        x61 = x + 4.5 * k + 2.5;  //左からの座標, 横の間隔
                        x61 = x + 4.5 * k + 30.2;  //左からの座標, 横の間隔
                        y61 = y + 15.12;  //上からの座標
                        x62 = x61 + 3.86;  //横3.86㎜
                        y62 = y61 + 3.86;  //縦3.86㎜
                        x61 = x61 + "mm";
                        ...
                        imgObj.place(selFile);  //入れる画像
                        imgObj.fit(FitOptions.frameToContent);  //内容にフレームサイズを合わせる
                        if (data[6+k].slice(0, -1) == "×"){  //×の場合
                            imgObj.transparencySettings.blendingSettings.opacity = 30;  //不透明度を30%に指定
                        }else{
                            imgObj.transparencySettings.blendingSettings.opacity =100;  //不透明度を100%に指定
                        }
                    }
                }
            }
